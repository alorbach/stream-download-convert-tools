...
class StreamDownloadConvertToolsUnifiedGUI(BaseAudioGUI):
    def __init__(self, root, auto_load_csv=None):
        super().__init__(root, "Stream Download Convert Tools - Unified")
    ...
    def setup_settings_tab(self):
        ...
        info_text = """Stream Download Convert Tools - Unified Application

Features:
- YouTube Downloader: Download videos from CSV lists
- Audio Modifier: Adjust speed and pitch of audio files
- Direct Link: Download from a single URL
- Automatic venv management via launcher scripts

Workflow:
1. Use YouTube Downloader or Direct Link to download videos
2. Use Video Tools (launchers/video_tools_unified.bat) to extract MP3, edit, or convert video
3. Use Audio Modifier to adjust speed/pitch

Note: FFmpeg is required for audio/video processing.
"""
...
def main():
    ...
    root = tk.Tk()
    app = StreamDownloadConvertToolsUnifiedGUI(root, auto_load_csv=auto_load_csv)
